...
StompFrame.prototype.toBuffer = function () {
  var commandBuf = new Buffer(this.command.toUpperCase(), 'utf8')
  var headersBuf = new Buffer(this.headersAsString(), 'utf8')
  var bodyBuf = new Buffer(this.body, 'utf8')
  var linefeedBuf = new Buffer([0x0A])
...
StompFrame.parse = function (buffer) {
  ...
  for (var lineNo = 0; start < buffer.length; lineNo++) {
    ...
    if (cr > 0 && cr === (lf - 1)) {
      ...
      throw new Error('Failed to parse line in STOMP frame')
    }
